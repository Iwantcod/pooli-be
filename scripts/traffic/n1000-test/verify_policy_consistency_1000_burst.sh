...
# -----------------------------------------------------------------------------
# Scenario A verifier: 1000-line burst stress consistency
# -----------------------------------------------------------------------------
...
G5_LINE_START=761
G5_LINE_END=880
VERIFY_WAIT_TIMEOUT_SECONDS="${VERIFY_WAIT_TIMEOUT_SECONDS:-600}"
...
app_id_mismatch_count="$(mysql_eval "SELECT COUNT(*) FROM TRAFFIC_DEDUCT_DONE WHERE line_id BETWEEN ${LINE_START} AND ${LINE_END} AND app_id != CASE WHEN line_id <= 400 THEN 1 WHEN line_id <= 600 THEN 2 WHEN line_id <= 760 THEN 3 WHEN line_id <= 880 THEN 2 ELSE 4 END;")"
