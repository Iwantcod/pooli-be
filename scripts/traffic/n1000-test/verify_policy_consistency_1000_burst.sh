...
app_id_mismatch_count="$(mongo_eval "const d=db.getSiblingDB('${MONGO_DB_NAME}'); const r=d.traffic_deduct_done_log.aggregate([{ \$match: { line_id: { \$gte: ${LINE_START}, \$lte: ${LINE_END} } } }, { \$project: { app_id: 1, expected_app: { \$switch: { branches: [ { case: { \$lte: ['\$line_id', 400] }, then: 1 }, { case: { \$lte: ['\$line_id', 600] }, then: 2 }, { case: { \$lte: ['\$line_id', 760] }, then: 3 }, { case: { \$lte: ['\$line_id', 880] }, then: 2 } ], default: 4 } } } }, { \$match: { \$expr: { \$ne: ['\$app_id', '\$expected_app'] } } }, { \$count: 'cnt' }]).toArray(); print(r.length === 0 ? 0 : Number(r[0].cnt));")"
