-- 1000-line traffic policy test setup (fixed group mapping)
...
--   G1_NO_RESTRICTION   : line 1~200    (family 1~50)    app_id=1
--   G2_LINE_DAILY_20MB  : line 201~400  (family 51~100)   app_id=1
--   G3_APP2_DAILY_5MB   : line 401~600  (family 101~150)  app_id=2
--   G4_SHARED_ONLY_APP3 : line 601~760  (family 151~190)  app_id=3
--   G5_APP2_SPEED_1MBPS : line 761~880  (family 191~220)  app_id=2
--   G6_APP4_DAILY_8MB   : line 881~1000 (family 221~250)  app_id=4
--
...
SELECT 'line_limit_g2_count' AS check_name, COUNT(*) AS cnt, 200 AS expected
FROM LINE_LIMIT
WHERE line_id BETWEEN @G2_START AND @G2_END;

SELECT 'app_policy_g3_count' AS check_name, COUNT(*) AS cnt, 200 AS expected
FROM APP_POLICY
...
SELECT 'app_policy_g5_count' AS check_name, COUNT(*) AS cnt, 120 AS expected
FROM APP_POLICY
WHERE line_id BETWEEN @G5_START AND @G5_END
  AND application_id = 2
  AND speed_limit = @APP2_SPEED_LIMIT_KBPS;

SELECT 'app_policy_g6_count' AS check_name, COUNT(*) AS cnt, 120 AS expected
FROM APP_POLICY
