...
# -----------------------------------------------------------------------------
# 1000-line policy test suite runner
# -----------------------------------------------------------------------------
...
echo "==============================================="
echo "Traffic Policy Suite (1000 lines)"
echo "==============================================="

echo "[A-1/3] Setup for Scenario A"
"$SCRIPT_DIR/setup_traffic_test_1000.sh"

echo "[A-2/3] Run Scenario A (Burst Stress)"
k6 run "$SCRIPT_DIR/k6_traffic_test_1000.js"

echo "[A-3/3] Verify Scenario A"
"$SCRIPT_DIR/verify_policy_consistency_1000_burst.sh"

echo "[B-1/3] Setup for Scenario B"
"$SCRIPT_DIR/setup_traffic_test_1000.sh"

echo "[B-2/3] Run Scenario B (Speed Exact)"
k6 run "$SCRIPT_DIR/k6_traffic_test_1000_speed_exact.js"

echo "[B-3/3] Verify Scenario B"
"$SCRIPT_DIR/verify_policy_consistency_1000_speed_exact.sh"
