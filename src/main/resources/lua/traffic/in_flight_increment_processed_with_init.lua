-- in_flight_increment_processed_with_init.lua
-- 목적:
-- - target processed 필드(processed_individual_data 또는 processed_shared_data)를 delta만큼 증가시킨다.
-- - 키가 없을 경우 기본 필드를 초기화한 뒤 증가한다.
-- - 현재 통합 차감 Lua는 차감과 processed_* 갱신을 내부에서 함께 수행하므로 이 스크립트를 직접 사용하지 않는다.
-- - legacy/수동 보정 경로의 원자 증가 helper로 남아 있으며, EM4 legacy 정리 단계에서 제거 여부를 결정한다.
--
-- 플로우:
-- 1) KEYS[1] 존재 여부 확인
-- 2) 미존재면 HSET(processed_individual_data=0, processed_shared_data=0, processed_qos_data=0, retry_count=0)으로 기본값 초기화
-- 3) HINCRBY target_processed_field delta 수행
-- 4) 증가 후 target_processed_field 값을 반환
--
-- 주의:
-- - 이 스크립트는 target processed 필드 증가 자체의 원자성만 보장한다.
-- - "차감 + processedData 갱신"을 동일 원자구간으로 보장하려면
--   차감 Lua 내부에 본 갱신을 포함하거나, 차감/갱신을 하나의 Lua로 통합해야 한다.
-- KEYS[1]: dedupe key
-- ARGV[1]: processed_individual_data field name
-- ARGV[2]: processed_shared_data field name
-- ARGV[3]: processed_qos_data field name
-- ARGV[4]: retry_count field name
-- ARGV[5]: initial value
-- ARGV[6]: target processed field name
-- ARGV[7]: processed delta
if redis.call('EXISTS', KEYS[1]) == 0 then
  redis.call('HSET', KEYS[1], ARGV[1], ARGV[5], ARGV[2], ARGV[5], ARGV[3], ARGV[5], ARGV[4], ARGV[5])
end
return redis.call('HINCRBY', KEYS[1], ARGV[6], ARGV[7])
