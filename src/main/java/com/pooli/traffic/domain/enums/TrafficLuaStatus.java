...
public enum TrafficLuaStatus {
    ...
    /** 전역 정책 스냅샷이 Redis에 없어 fallback 정책 hydrate 후 재시도가 필요한 상태입니다. */
    GLOBAL_POLICY_HYDRATE,

    /** 월별 개인/공유 잔량 또는 QoS hash가 Redis에 없어 fallback 잔량 hydrate 후 재시도가 필요한 상태입니다. */
    HYDRATE,

    /** 월별 개인 잔량 snapshot(`amount`, `qos`)이 Redis에 없어 fallback 개인 hydrate 후 재시도가 필요한 상태입니다. */
    HYDRATE_INDIVIDUAL,

    /** 월별 공유 잔량 snapshot(`amount`)이 Redis에 없어 fallback 공유 hydrate 후 재시도가 필요한 상태입니다. */
    HYDRATE_SHARED,
